...
async def run_case(name,record=True):
 ...
 folder=root/'tests/navigation/scanned_chair'/'final_presentation';folder.mkdir(parents=True,exist_ok=True)
 async def frames(n):
 ...
 filename=root/'isaacsim/scenes/navbot_scanned_chair.usda'
 targets=['/World/SuiteObstacle_0']
 await ctx.open_stage_async(str(filename));await frames(90)
 ...
 vp=get_active_viewport();vp.set_active_camera('/World/ObservationCamera')
...
if __name__=='__main__':asyncio.ensure_future(run_case('chair',record=True))
