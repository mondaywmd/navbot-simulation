"""Isaac Sim Script Editor: ten seeded depth-assisted navigation tests.
...
CASES={}
SUITE_SEEDS=list(range(2026092601,2026092611))

async def run_case(name,seed,blocked_case=False,record=True):
 ...
 folder=root/'tests/navigation/random_suite_20260926_v3_14cm'/name;folder.mkdir(parents=True,exist_ok=True)
 async def frames(n):
 ...
 # Keep physical clearance along one corridor side; randomize order, position and yaw.
 side=1 if seed%2 else -1
 for i,(asset,y) in enumerate(zip(names,[-2.2,-1.45,-.7,0.])):
  x=(-.055 if side==1 else -.515)+rng.uniform(-.025,.025);y+=rng.uniform(-.05,.05);angle=rng.uniform(-10,10)
 ...
 if blocked_case:
  barrier=UsdGeom.Cube.Define(st,'/World/BlockageControl');barrier.CreateSizeAttr(1);xf=UsdGeom.XformCommonAPI(barrier);xf.SetTranslate(Gf.Vec3d(-.285,-1.8,.175));xf.SetScale(Gf.Vec3f(1.05,.12,.35));barrier.CreateDisplayColorAttr([Gf.Vec3f(.65,.16,.06)]);UsdPhysics.CollisionAPI.Apply(barrier.GetPrim());targets.append('/World/BlockageControl');layout.append({'asset':'full_width_control_barrier','x':-.285,'y':-1.8,'dimensions_m':[1.05,.12,.35]})
 CASES[name]=layout
 filename=root/'isaacsim/scenes'/('navbot_random_v3_'+name+'.usda');st.GetRootLayer().Export(str(filename));await ctx.open_stage_async(str(filename));await frames(90)
 ...
 vp=get_active_viewport();cfg=json.loads((root/'tests/navigation/scanned_chair/observation_camera.json').read_text(encoding='utf-8'));cam=UsdGeom.Camera.Define(s,'/World/NavTestCamera');cam.CreateFocalLengthAttr(cfg['focal_length']);cam.CreateClippingRangeAttr(Gf.Vec2f(.01,100));cx=UsdGeom.Xformable(cam);cx.AddTransformOp().Set(Gf.Matrix4d(*[v for row in cfg['world_transform'] for v in row]));vp.set_active_camera('/World/NavTestCamera');s.GetRootLayer().Save();await frames(15)
 ...
 try:
  while t.get_current_time()-begin<100:
   ...
   if g.numElements and g.scanComplete and g.frameId!=last:
    ...
    clearance=distance_transform_edt(~grid)*resolution;blocked=clearance<.14
  ...
  expected_stop=reason in ['no_map_path','blocked_no_progress','no_safe_gap'] and final[1]<-1.98
  passed=(expected_stop if blocked_case else reason=='goal_reached') and max_force<=.1 and drift<.03
  ...
  report=dict(case=name,seed=seed,expected_outcome="safe_stop" if blocked_case else "goal_reached",passed=locals().get('passed',False),reason=reason,goal=goal.tolist(),obstacles=CASES[name],final=locals().get('final'),duration_s=locals().get('stop_time'),braking_drift_m=locals().get('drift'),max_contact_force_n=max_force,contact_body_count=contact.sensor_count,monitored_colliders=filters,trajectory=history,video_frames=captures,localization='simulator ground truth',depth_obstacle_grid_counts=depth_counts,depth_ground_threshold_m=.012,depth_memory_seconds=3,planner='A-star on accumulated sensor points, 2.5 cm grid, 14 cm footprint inflation')
  (folder/'report.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
 print(json.dumps({k:v for k,v in report.items() if k not in ['trajectory','video_frames','monitored_colliders']}))
 return report

async def main():
 root=Path('C:/Users/User/Documents/navbot/_isaacsim');out=root/'tests/navigation/random_suite_20260926_v3_14cm';out.mkdir(parents=True,exist_ok=True)
 ctx=omni.usd.get_context();ctx.get_stage().Export(str(out/'before_suite.usda'))
 reports=[]
 for i,seed in enumerate(SUITE_SEEDS):
  name=f'{i+1:02d}_'+('blocked' if i>=8 else 'passage')
  (out/'status.json').write_text(json.dumps({'state':'running','case':name,'completed':len(reports),'total':10}),encoding='utf-8')
  try:
   r=await run_case(name,seed,blocked_case=i>=8,record=False);reports.append({k:v for k,v in r.items() if k not in ['trajectory','video_frames','monitored_colliders','depth_obstacle_grid_counts']})
  except Exception as error:
   import traceback
   omni.timeline.get_timeline_interface().pause();(out/'error.txt').write_text(traceback.format_exc(),encoding='utf-8');(out/'status.json').write_text(json.dumps({'state':'error','case':name,'error':str(error),'completed':len(reports)}),encoding='utf-8');raise
  (out/'summary.json').write_text(json.dumps(reports,indent=2),encoding='utf-8')
 (out/'status.json').write_text(json.dumps({'state':'complete','completed':len(reports),'passed':sum(r['passed'] for r in reports),'total':10}),encoding='utf-8')

if hasattr(builtins,'navbot_suite_task') and not builtins.navbot_suite_task.done():raise RuntimeError('A suite is already running')
builtins.navbot_suite_task=asyncio.ensure_future(main())
print('Started 10-case suite; progress in tests/navigation/random_suite_20260926_v3_14cm/status.json')
