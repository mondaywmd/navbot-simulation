...
async def run_case(name,seed,blocked_case=False,record=True):
 ...
 folder=root/'tests/navigation/random_suite_20260926_v2'/name;folder.mkdir(parents=True,exist_ok=True)
 async def frames(n):
 ...
 CASES[name]=layout
 filename=root/'isaacsim/scenes'/('navbot_random_'+name+'.usda');st.GetRootLayer().Export(str(filename));await ctx.open_stage_async(str(filename));await frames(90)
...
async def main():
 root=Path('C:/Users/User/Documents/navbot/_isaacsim');out=root/'tests/navigation/random_suite_20260926_v2';out.mkdir(parents=True,exist_ok=True)
 ...
 for i,seed in enumerate(SUITE_SEEDS):
  ...
  try:
   r=await run_case(name,seed,blocked_case=i>=8,record=True);reports.append({k:v for k,v in r.items() if k not in ['trajectory','video_frames','monitored_colliders','depth_obstacle_grid_counts']})
...
print('Started 10-case suite; progress in tests/navigation/random_suite_20260926_v2/status.json')
